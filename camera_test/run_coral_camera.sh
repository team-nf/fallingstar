...
# This script runs the Coral TPU camera detection using Docker Compose
...
# Run the detection script using Docker Compose
docker compose run --rm coral python3 coral_camera_detection.py \
    --model "$MODEL_PATH" \
    --labels "$LABELS_PATH" \
    --threshold "$THRESHOLD" \
    --camera "$CAMERA_ID" \
